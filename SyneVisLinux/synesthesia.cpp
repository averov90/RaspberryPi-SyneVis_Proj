...
static void pa_asunc_calc() {
	...
	Mat tmp[3];
	unique_lock<mutex> locker(pasunccalc); //If not work: paste this lite at first to while
	while (true)
	{
...
char Syne_Initialise() {
	if (!WavUtils::LoadSNDPacket("/home/linaro/projects/SyneVisLinux/synesthesia/collect.txt", "/home/linaro/projects/SyneVisLinux/synesthesia/sndfolder", SynesthesyData)) return 1;
